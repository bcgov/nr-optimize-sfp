...
min_CAD <- data.frame(list("Category" = "CAD",
                ...
                "Total size (GB)" = dollar(sum(CAD$CAD_Size_GB)), 
                "Total cost" = sum(CAD$Cost),
...
min_Documents <- data.frame(list("Category" = "Documents",
                        "File count" = sum(Documents$Documents_Images_File_Count),       
...
min_category_df <- rbind(min_AppData, min_Archive, min_Audio, min_Backups, min_CAD, min_Database, min_Disk_Images, min_Email, min_Empty_Extension, min_Encase, min_Executables, min_Images, min_Map, min_P2P, min_Source_Code, min_System, min_Temporary, min_Video, min_Web_Page)
```

```{r, categorical top ten search, include = FALSE}
...
`r datatable(min_category_df, rownames = FALSE, colnames = c('Category' = 1, 'File Count' = 2, 'Total Size(GB)' = 3, 'Cost' = 4, 'Avg GB per User' = 5), options=list(columnDefs = list(list(className = 'dt-left', targets="_all"))))`
<br>
<br>
