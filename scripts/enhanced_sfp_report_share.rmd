...
formatted_sfp_data <- sfp_data %>%
  ...
  mutate(
    root  = stringr::word(path, 1, 7, sep = "\\\\"),
    sizegb = round(sizemb / 1000, 3),
    filetype = tolower(stringr::str_extract(filename, "[^.]+$")),
    monthly_cost = round(sizegb * 2.7, 2),
    obj_store_cost = sizegb * 0.07,
    object_storage_cost = round(obj_store_cost, 2),
...
```{r find paths exceeding 260 characters, include = FALSE}
formatted_sfp_data$char_count <- nchar(formatted_sfp_data$path)-29
```
...
# Count backslashes in each UNC path
table_pathsize_tallyfiles$folderdepth <- stringi::stri_count_fixed(
  table_pathsize_tallyfiles$path,
  "\\"
)

# Count backslashes in the share root
path_depth <- stringi::stri_count_fixed(
  params$share,
  "\\"
)

# subtract path parameter count from each path name's count for a folder depth that starts at the desired search folder
table_pathsize_tallyfiles$folderdepth <- (table_pathsize_tallyfiles$folderdepth - path_depth) - 4
...
```{r find older files, include = FALSE}
# convert the data collected text to date, subtract time for "last accessed" preference
...
\section{Older Files}
**`r la_percent`%** of files over 1MB were last accessed **`r file_age_label(params$ministry)`** ago or longer \textit{(`r {la_old_count}` files)}. This represents \textbf{`r {la_old_gb_num}` GB} of data at a monthly cost of \fontspec[Color=Red]{Verdana Bold}`r {la_old_cost}`\fontspec[Color=Black]{Verdana}.
\clearpage
...
- [Fiscal Year 2026-2027 Chart of NRM SFP Consumption](http://nrmccp-metabase.apps.silver.devops.gov.bc.ca/public/dashboard/0bd8479c-fb25-4001-abdd-362eee0ae251) 
...
addStyle(excel, sheet = 4, sty1, rows=2:(nrow(sfp_categories_filetypes)+1), cols=5)
