...
params:
  ...
  ministry:
    ...
    value: BCWS
    input: select
    choices: [AF, BCWS, ECS, ENV, FOR, IRR, MCM, WLRS]
  collected:
...
\section{Older Files}
**`r la_percent`%** of files over 1MB were last accessed at least **`r file_age_label(params$ministry)`** ago \textit{(`r {la_old_count}` files)}. This represents \textbf{`r {la_old_gb}` GB} of data at a monthly cost of \fontspec[Color=Red]{Verdana Bold}`r {la_old_cost}`\fontspec[Color=Black]{Verdana}.
\clearpage
...
#dt.worksheets(4, files_accessed_old)
...
#addStyle(excel, sheet = 4, sty1, rows=2:(nrow(files_accessed_old)+1), cols=5)
